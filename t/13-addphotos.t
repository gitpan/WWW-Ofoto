...
is $result, scalar @$pics, "successfully upload to new albm";

$result = $ofoto->upload_to_album( {
		title => $title,
		pics  => $pics,
	} );
is $result, scalar @$pics, "successfully added to new albm";

my $albums = $ofoto->list_albums;
ok $albums->{$title}, "list albums found new album";
is $albums->{$title}->{count}, (scalar @$pics)*2, "correct count returned";
